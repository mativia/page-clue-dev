...
// Access Key pública de https://web3forms.com (es pública, no secreta).
...
export default function Contact() {
  ...
  // Guardamos el índice, no la etiqueta: así el mail que les llega va
  // siempre en español aunque el visitante navegue en inglés.
  const [servicioIdx, setServicioIdx] = useState(null)
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          Rol: fields.rol,
          Email: fields.email,
          Servicio: COPY.es.contact.services[servicioIdx],
      ...
      if (data.success) {
        ...
        setStatus('error')
        setMessage(data.message || c.errGeneric)
      }
    } catch {
      setStatus('error')
      setMessage(c.errNetwork)
    }
  }

  return (
    <section className={styles.section} id="contacto" aria-label={c.eyebrow}>
      ...
      >
        ...
        <div className={styles.formSide}>
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <div className={styles.formField}>
              ...
              <div className={styles.chips} role="radiogroup" aria-label={c.radiogroupLabel}>
                {c.services.map((s, i) => (
                  <button
                    type="button"
                    key={s}
                    role="radio"
                    aria-checked={servicioIdx === i}
                    className={`${styles.chip}${servicioIdx === i ? ` ${styles.chipActive}` : ''}`}
        ...
        {/* ── Panel "qué pasa después" ── */}
        <aside className={styles.infoSide}>
          <p className={styles.intro}>{c.intro}</p>

          <div className={styles.infoBlock}>
            <h3 className={styles.infoTitle}>{c.nextTitle}</h3>
          ...
          <div className={styles.infoBlock}>
            <h3 className={styles.infoTitle}>{c.directTitle}</h3>
            <div className={styles.contacts}>
              ...
              <a href={PHONE.href} className={styles.contactItem}>{PHONE.display}</a>
            </div>
          </div>

          <div className={styles.infoBlock}>
            <h3 className={styles.infoTitle}>{c.socialsTitle}</h3>
